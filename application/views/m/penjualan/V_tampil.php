<div style="margin-top: 80px;">
	<div style="margin: 0px 25px;">
		<div class="panel panel-default">
			<div class="panel-heading">Penjualan Transaksi</div>
				...
				<form id="res-nama" method="post" action="">
					...
					<div class="form-row">
						...
						<div class="form-group col-md-3 form-save_all" style="margin-left: 600px;">
							<label class="lbl label-save_all">Kode Pelanggan</label>
							
							<select name="kodepel" id="kodepel" class="selectpicker form-control" data-live-search="true" placeholder="Kode Pelanggan" required>
								<option></option>
								<?php 
									foreach($option as $row_option){
									?>
									<option value="<?php echo $row_option['plg_id']; ?>"><?php echo $row_option['plg_id']; ?></option>
									<?php
									}
								?>
							</select>
					...
					<div class="form-row">
						...
						<div class="form-group col-md-3 form-save_all" style="margin-left: 600px;">
							<label class="lbl label-save_all">Nama Pelanggan</label>
							<input type="text" class="form-control" id="namapel" name="namapel" placeholder="Nama Pelanggan" readonly>
						</div>
					</div>
				</form>
			</div>
		...
		<div class="panel panel-default">
			<div class="panel-heading">Table Barang Penjualan</div>
			<div class="panel-body">
		...
		<div class="panel panel-default">
			...
			<div class="panel-body">
				...
				<div class="frm_btn_pnjl" style="display:none;">
...
<script type="text/javascript">
	...
	$(document).ready( function () {
		table = $('#table').DataTable({ 
			...
			"bLengthChange": false,
			"ajax": {
				"url": '<?php echo site_url('m/C_penjualan/table_list'); ?>',
				"type": "POST"
			},
			"columns": [
			{"data": "kodebar"},
		...
		$('#barang').change(function(){
			$.ajax({
				url: '<?php echo site_url("m/C_penjualan/getOptionSelectBarang")?>',
				type: "GET",
				dataType: "json",
				data: {nama:$(this).val()},
		...
		$('#kodepel').change(function(){
			$.ajax({
				url: '<?php echo site_url("m/C_penjualan/getOptionSelectKodepel")?>',
				type: "GET",
				dataType: "json",
				data: {kode:$(this).val()},
				success: function(data){
	...
	{	
		...
		$.ajax({
			url : "<?php echo site_url('m/C_penjualan/save_all');?>",
			type: "POST",
			...
			success: function(data)
			{
				if(data.status){
					...
					{
						$('.form-save_all').addClass('has-error'); //select parent twice to select div form-group class and add has-error class
						$('.label-save_all').addClass('text-danger'); //select parent twice to select div form-group class and add has-error class
	...
	setInterval(function() {
		$.ajax({
			method: 'GET',
			url: '<?php echo site_url('m/C_penjualan/total_bayar');?>',
	...
	setInterval(function() {
		$.ajax({
			method: 'GET',
			url: '<?php echo site_url('/m/C_penjualan/nofak'); ?>',
			success: function(text) {
				$('[name="nofak"]').val(text);
			}
		});
	...
	{	
		...
		$.ajax({
			url : "<?php echo site_url('m/C_penjualan/add_barang')?>",
			type: "POST",
	...
	{
		$.ajax({
			url : "<?php echo site_url('m/C_penjualan/delete_item')?>/" + id,
			type: "POST",
			dataType: "JSON",
			success: function(data)
			{
	...
	function delete_item_all()
	{
		...
		modalConfirm(function(confirm){
			if(confirm){
				$.ajax({
					url : "<?php echo site_url('m/C_penjualan/delete_item_all')?>",
					type: "POST",
					dataType: "JSON",
					success: function(data)
					{
				...
				<form action="#" id="form_barang" class="form-horizontal">
					<div class="form-body">
						...
						<div class="form-group">
							...
							<div class="col-md-5">
								<input name="hgj" id="jual" placeholder="Harga Jual" class="form-control" type="text" readonly>
							</div>
						</div>
